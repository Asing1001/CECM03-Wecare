...
namespace MyProgram
{
    public partial class frmCounselorHomePage : MyClassLibrary.frmLogo
    {
        clsUser user = null;
        string[] strFunctions = { "行事曆", "基本資料", "離開系統" };

        public frmCounselorHomePage(clsUser user)
        {
            InitializeComponent();
            ...
            btn_MyCalendar(new object(), new EventArgs());
        ...
        private void btn_MyCalendar(object sender, EventArgs e)
        {
            if (this.splitContainer2.Panel2.Controls.Count > 0)
            {
                foreach (Control each in this.splitContainer2.Panel2.Controls)
                {
                    if (each is frmCalendar)
                    {
                        each.BringToFront();
                        return;
                    }
            ...
            frmCalendar f = new frmCalendar(user);
            this.showForm(f);
        }
